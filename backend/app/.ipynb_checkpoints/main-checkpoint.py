...
from .agent_bedrock import ContextGraphAgentBedrock
...
async def lifespan(app: FastAPI):
    ...
    if context_graph_client.verify_connectivity():
        ...
        if gds_client:
            ...
            logger.info("GDS not available - skipping GDS analyses refresh")
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        logger.info(f"Creating agent (Bedrock: {config.use_bedrock})...")
        AgentClass = ContextGraphAgentBedrock if config.use_bedrock else ContextGraphAgent
        async with AgentClass() as agent:
...
async def chat_stream(request: ChatRequest):
    ...
    async def event_generator():
        try:
            ...
            logger.info(f"Creating agent for streaming (Bedrock: {config.use_bedrock})...")
            AgentClass = ContextGraphAgentBedrock if config.use_bedrock else ContextGraphAgent
            async with AgentClass() as agent:
...
async def find_similar_decisions(decision_id: str, limit: int = 5):
    ...
    if not gds_client:
        raise HTTPException(status_code=503, detail="GDS not available. Requires Neo4j AuraDS or Enterprise with GDS plugin.")
    try:
...
async def detect_fraud_patterns(
    account_id: str | None = None, similarity_threshold: float = 0.7
):
